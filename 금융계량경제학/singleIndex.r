...
loadPath = "C:\\Users\\sendt\\Desktop\\Jane\\Class\\Under_Econometrics\\"
...
td = seq(as.Date("2021-11-10"), as.Date("2022-11-08"), by="day")
...
par(mfrow=c(2,2))
	hist(si.df$sp500,main="S&P 500 daily cc returns",
...
main="Daily cc returns on S&P 500 and Starbucks",
...
main="Daily cc returns on S&P 500 and Microsoft",
ylab="returns", col=c("blue","orange"), lwd=c(2,2))
abline(h=0)
...
main="Daily cc returns on S&P 500 and Nordstrom",
ylab="returns", col=c("blue","orange"), lwd=c(2,2))
abline(h=0)
...
# sp500 vs. boeing
plot(si.z[,c("sp500","ba")], plot.type="single",
main="Daily cc returns on S&P 500 and Boeing",
ylab="returns", col=c("blue","orange"), lwd=c(2,2))
abline(h=0)
legend(x="bottomright",legend=c("S&P 500","BOEING"),
       lwd=c(2,2),col=c("blue","orange"))

plot(si.df$sp500,si.df$ba, col="slateblue1",lwd=2,pch=16,cex=2,
     main="Daily cc returns on S&P 500 and Boeing",
     xlab="cc return on S&P 500", ylab="cc return on boeing")
abline(h=0,v=0)
abline(a=alpha.vals["ba"],b=beta.vals["ba"], 
       col="orange", lwd=3)
...
boeing.fit = lm(ba~sp500,data=si.df)
...
port = (si.df$msft + si.df$sbux + si.df$nord + si.df$ba)/4
si.df$port = port
colnames(si.df)
...
chart.RollingRegression(si.z[,"ba",drop=F], si.z[,"sp500",drop=F], width=24)
abline(h=beta.vals["ba"], lwd=2, col="red")
